...
export function normalizeSearchKeyword(keyword: string): string {
  ...
  normalized = normalized.split('').map(ch => vietnameseMap[ch] || ch).join('');
...
/**
 * Loại bỏ dấu tiếng Việt nhưng giữ nguyên khoảng trắng
 */
export function removeVietnameseDiacritics(text: string): string {
  ...
  return text.split('').map(ch => vietnameseMap[ch] || ch).join('');
}

/**
 * Match code của category: match chính xác hoặc match dạng {keyword}-*
 * Ví dụ: tìm "GiaiPhau" sẽ match với "GiaiPhau", "GiaiPhau-HMU", "GiaiPhau-YDN", "GiaiPhau-TUMP"
 */
export function matchesCategoryCode(code: string, keyword: string): boolean {
  if (!code || !keyword) return false;
  
  const normalizedCode = normalizeSearchKeyword(code);
  const normalizedKeyword = normalizeSearchKeyword(keyword);
  
  // Match chính xác hoặc match dạng {keyword}-*
  return normalizedCode === normalizedKeyword || normalizedCode.startsWith(`${normalizedKeyword}-`);
}

/**
 * Match title của category với wild card matching
 */
export function matchesCategoryTitle(title: string, keyword: string): boolean {
...
/**
 * Tạo slug từ code và title của subcategory
 */
export function createSubCategorySlug(code: string, title: string): string {
  const vietnameseMap: Record<string, string> = {
    'à': 'a', 'á': 'a', 'ạ': 'a', 'ả': 'a', 'ã': 'a',
    'â': 'a', 'ầ': 'a', 'ấ': 'a', 'ậ': 'a', 'ẩ': 'a', 'ẫ': 'a',
    'ă': 'a', 'ằ': 'a', 'ắ': 'a', 'ặ': 'a', 'ẳ': 'a', 'ẵ': 'a',
  ...
  return `${code}-${toSlug(title)}`;
}

/**
 * Danh sách màu gradient cho biodigital cards
 */
...
/**
 * Lấy màu gradient random dựa trên title (để đảm bảo cùng title luôn có cùng màu)
 */
export function getRandomGradientColor(title: string): string {
  if (!title) return GRADIENT_COLORS[0];
  
  // Tạo hash từ title để đảm bảo cùng title luôn có cùng màu
  let hash = 0;
...
/**
 * Tạo slug từ title (dùng cho biodigital categories)
 */
